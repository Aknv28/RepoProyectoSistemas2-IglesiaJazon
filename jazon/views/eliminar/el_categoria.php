...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql_update = "UPDATE categoria SET habilitado = 0 WHERE id_categoria = :id_categoria";
    $stmt = $conn->prepare($sql_update);
    $stmt->bindParam(':id_categoria', $id_categoria);

    if ($stmt->execute()) {
        echo "<script>
                alert('Categoria deshabilitado exitosamente');
                window.location.href = '../listas/lst_categorias.php';  // Redirigir a la página de listado o similar
              </script>";
    } else {
        echo "<div class='alert alert-danger text-center'>Error al deshabilitar categoria: " . $conn->errorInfo()[2] . "</div>";
    }
} else {
